...
// disposition table). Do not build new functionality on top of this file —
// it is a deliberately temporary bridge, not a second wire format going
// forward.
//
...
inline const std::error_category& wire_category() noexcept {
    struct Cat : std::error_category {
        const char* name() const noexcept override { return "rcp.legacy_wire"; }
        std::string message(int ev) const override {
            switch (static_cast<WireErrc>(ev)) {
            case WireErrc::short_frame: return "rcp/legacy_wire: frame too short";
            case WireErrc::bad_magic:   return "rcp/legacy_wire: bad magic bytes";
            case WireErrc::bad_version: return "rcp/legacy_wire: unsupported protocol version";
            default:                    return "rcp/legacy_wire: unknown error";
            }
        }
    };
    static Cat instance;
