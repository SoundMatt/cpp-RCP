...
// DoIP (Diagnostics over IP / ISO 13400) bridge interface stub.
//
// Encapsulates UDS requests inside DoIP diagnostic messages over TCP/IP.
// Requires a DoIP stack integration. All methods return
...
// Disposition table's entry for `doipbr.hpp` — translating between an RC
// Client's application-level view and a DoIP diagnostic session is
...
// `DoIpController : public rcp::Controller` wrapper (keyed by the removed
// `Zone` type) is dropped for a standalone `DoIpBridge`, the same
...
// Phase 14/15 header keys on since v2.10.0. `DoIpBridge::request` takes on
...
//
// This header bridges into DoIP's TCP/IP diagnostic-session encapsulation;
// it is unrelated to `rcp/udsbr.hpp`'s UDS service bridge beyond the fact
// that DoIP commonly carries UDS requests — each header stubs its own side
// of that pairing independently.
#pragma once
...
// DoIpBridge stubs the request/response side of a bridge between one RCP
// endpoint (`stream_key` + `endpoint`) and a DoIP diagnostic session
// against `cfg.server_ip`:`cfg.server_port`. No concrete DoIP stack is
// linked here; every call reports errc::function_not_supported.
class DoIpBridge {
public:
    DoIpBridge(uint64_t stream_key, avtp::ByteBusId endpoint, Config cfg)
...
inline std::shared_ptr<DoIpBridge> new_bridge(uint64_t stream_key, avtp::ByteBusId endpoint, Config cfg) {
    return std::make_shared<DoIpBridge>(stream_key, endpoint, std::move(cfg));
}
