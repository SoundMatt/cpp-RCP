...
// DDS (Data Distribution Service) protocol bridge interface stub.
//
// Publishes RC-Client-level requests as DDS typed topics and collects
// replies from a dedicated reply topic. Requires an OMG DDS implementation
// (e.g. FastDDS, Cyclone DDS) to be linked separately. All methods return
...
// Disposition table's entry for `ddsbr.hpp` — translating between an RC
// Client's application-level view and the DDS ecosystem is conceptually
...
// `DdsController : public rcp::Controller` wrapper (keyed by the removed
// `Zone` type) is dropped for a standalone `DdsBridge`, the same
...
// Phase 14/15 header keys on since v2.10.0. `DdsBridge::request` takes on
...
// DdsBridge stubs the request/response side of a bridge between one RCP
// endpoint (`stream_key` + `endpoint`) and its corresponding pair of DDS
// topics under `cfg.topic_prefix` in DDS domain `cfg.domain_id`. No
// concrete DDS implementation is linked here; every call reports
// errc::function_not_supported.
class DdsBridge {
public:
    DdsBridge(uint64_t stream_key, avtp::ByteBusId endpoint, Config cfg)
...
inline std::shared_ptr<DdsBridge> new_bridge(uint64_t stream_key, avtp::ByteBusId endpoint, Config cfg) {
    return std::make_shared<DdsBridge>(stream_key, endpoint, std::move(cfg));
}
