...
// Tests for rcp/legacy_mock.hpp — moved unchanged from tests/test_mock.cpp
// (ROADMAP.md milestone 56, "Test & Simulation Harness Rebuild", v2.12.0)
// when rcp/mock.hpp itself became the new RC Server simulator; only the
// mock:: -> legacy_mock:: namespace rename applies here, same as
// rcp/legacy_wire.hpp's tests/test_legacy_wire.cpp precedent at v2.0.0. See
// tests/test_mock.cpp for the new mock::Server coverage.
...
TEST_CASE("Registry::register_ctrl returns ErrAlreadyExists for duplicate zone", "[registry][REQ-REG-002]") {
    auto reg  = legacy_mock::new_registry();
    auto dup  = std::make_shared<legacy_mock::Controller>(Zone::FrontLeft);
...
TEST_CASE("Registry::register_ctrl returns ErrClosed after close", "[registry][REQ-REG-007]") {
    auto reg   = legacy_mock::new_registry();
    reg->close();
    auto extra = std::make_shared<legacy_mock::Controller>(Zone::FrontLeft);
...
TEST_CASE("Registered controller is immediately retrievable via lookup", "[registry][REQ-REG-009]") {
    auto reg = legacy_mock::new_registry();
    REQUIRE_FALSE(reg->deregister(Zone::FrontLeft));
    auto nc  = std::make_shared<legacy_mock::Controller>(Zone::FrontLeft);
...
TEST_CASE("Controller::send returns StatusOK with no handler", "[controller][REQ-CTRL-001]") {
    legacy_mock::Controller ctrl(Zone::FrontLeft);
    Response         out;
...
TEST_CASE("Controller::send dispatches to handler and returns response verbatim", "[controller][REQ-CTRL-002][REQ-CTRL-016]") {
    bool called = false;
    legacy_mock::Controller ctrl(Zone::FrontLeft, [&](const Command& cmd) -> Response {
        called = true;
...
TEST_CASE("Controller::send returns ErrTimeout when ctx is already done", "[controller][REQ-CTRL-004][REQ-CTRL-023]") {
    ...
    legacy_mock::Controller ctrl(Zone::FrontLeft, [&](const Command&) -> Response {
        handler_called = true;
        return {};
    });
...
TEST_CASE("Controller::subscribe channel closes when controller closes", "[controller][REQ-CTRL-007]") {
    auto ctrl = std::make_unique<legacy_mock::Controller>(Zone::FrontLeft);
...
TEST_CASE("Status.Zone is correct for every published update", "[controller][REQ-CTRL-020][REQ-STAT-001]") {
    legacy_mock::Controller ctrl(Zone::RearLeft);
    std::shared_ptr<StatusChannel> ch;
    REQUIRE_FALSE(ctrl.subscribe(Context::background(), ch));
    ctrl.publish({});
    auto s = ch->recv();
...
TEST_CASE("Payload is copied before passing to handler", "[controller][REQ-CTRL-026]") {
    ...
    legacy_mock::Controller ctrl(Zone::FrontLeft, [&](const Command& c) -> Response {
