...
using namespace rcp;
using namespace rcp::legacy_wire;

// ── Header constants ──────────────────────────────────────────────────────────

TEST_CASE("Legacy wire header length and magic are correct", "[legacy_wire][REQ-UDP-001]") {
    REQUIRE(HeaderLen == 16);
    REQUIRE(MagicByte0 == 'R');
    REQUIRE(MagicByte1 == 'C');
}

TEST_CASE("Legacy wire version is 1", "[legacy_wire][REQ-UDP-002]") {
    REQUIRE(ProtoVer == 1);
}

TEST_CASE("Legacy MaxPayload is non-zero", "[legacy_wire][REQ-UDP-003]") {
    REQUIRE(MaxPayload > 0);
}

// ── Command round-trip ────────────────────────────────────────────────────────

TEST_CASE("encode_command / decode_command round-trip with no payload", "[legacy_wire][REQ-UDP-004]") {
    Command cmd;
    cmd.id       = 0xDEADBEEF;
...
TEST_CASE("encode_command / decode_command round-trip with payload", "[legacy_wire][REQ-UDP-005]") {
    Command cmd;
    cmd.id      = 42;
...
TEST_CASE("decode_command rejects truncated frames", "[legacy_wire][REQ-UDP-006]") {
    Command cmd;
    cmd.id   = 1;
...
TEST_CASE("encode_response / decode_response round-trip", "[legacy_wire][REQ-UDP-007]") {
    Response resp;
...
TEST_CASE("encode_status / decode_status round-trip", "[legacy_wire][REQ-UDP-008]") {
    Status st;
...
TEST_CASE("Frames begin with 'RC' magic and correct version", "[legacy_wire][REQ-UDP-009]") {
    Command cmd;
    cmd.zone = Zone::FrontLeft;
    auto frame = encode_command(cmd);
    REQUIRE(frame[0] == 'R');
...
TEST_CASE("decode_command rejects wrong magic", "[legacy_wire][REQ-UDP-010]") {
    Command cmd;
    cmd.zone = Zone::FrontLeft;
    auto frame = encode_command(cmd);
...
TEST_CASE("decode_command rejects wrong version", "[legacy_wire][REQ-UDP-011]") {
    Command cmd;
    cmd.zone = Zone::FrontLeft;
    auto frame = encode_command(cmd);
...
TEST_CASE("encode_control produces a valid header-only frame", "[legacy_wire][REQ-UDP-012]") {
